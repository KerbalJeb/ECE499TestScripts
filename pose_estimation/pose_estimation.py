...
#         [      0,       0,       1],
#         ])
# #When my mobile phone shoots the chessboard, the image size is 4000 x 2250
# #ip camera is set to 1920 x 1080 when shooting video, the aspect ratio is the same,
# #Note when setting the resolution of the camera
#
#
...
while True:
    ...
    #Use the aruco.detectMarkers() function to detect the marker and return the ID and the 4 corner coordinates of the marker board
...
# If you can't find the id
    if ids is not None:
        ...
        # Estimate the pose of each marker and return the values ​​rvet and tvec --- different
        # from camera coeficcients
        (rvec-tvec).any() # get rid of that nasty numpy value array error

# aruco.drawAxis(frame, mtx, dist, rvec, tvec, 0.1) #Draw axis
# aruco.drawDetectedMarkers(frame, corners) #Draw a square around the marker
    ...
    if key == ord(' '):   # Press the space bar to save
#        num = num + 1
